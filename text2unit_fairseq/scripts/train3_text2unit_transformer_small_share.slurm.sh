...
for icn in ${!layerdrops[@]}; do
    layerdrop=${layerdrops[$icn]}

    for ild in ${!clip_norms[@]}; do
        clip_norm=${clip_norms[$ild]}
        ...
        for i in ${!dropouts[@]}; do
            ...
            for j in ${!max_tokenss[@]}; do
                ...
                project=${arch}-dataset_${dataset}-dropout_${dropout}-layerdrop_${layerdrop}-max_tokens_${max_tokens}-clip_norm_${clip_norm}-share
