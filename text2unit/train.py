...
def main(args):
    ...
    val_dataloader = from_path(args.val_txt_path, args.val_batch_size, "val", num_workers=args.num_workers, is_distributed=False) # not used for now
    # not in args
    ...
    for epoch in epoch_bar:
        ...
        if (len(epoch_accs) - np.argmax(epoch_accs)) > 25:
            break
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_len_src', type=int, default=500, help='only used for training')
    parser.add_argument('--max_len_trg', type=int, default=1000, help="only used for training")
