...
def infer(data_dir, 
          batch_infer=False, 
          batch_size=True,
          source="/home/junkaiwu/speechbrain/asr-transformer-transformerlm-librispeech",
          savedir="/home/junkaiwu/speechbrain/asr-transformer-transformerlm-librispeech"):
    
    asr_model = EncoderDecoderASR.from_hparams(source=source, savedir=savedir, run_opts={"device":"cuda"})
    
    print("ASR Model Loaded!")
    audio_files = librosa.util.find_files(data_dir, ext=["wav"], recurse=False)
    ...
    if batch_infer:
        ...
        batches = int(np.ceil(len(audio_files) / batch_size))
        
        for i in tqdm(range(batches)):
            indices = audio_orders[i*batch_size:(i+1)*batch_size]
            batch_files = []
            for idx in indices:
                ...
                for audio_file, pred in zip(batch_files, preds):
                    preds_dict[basename(audio_file).strip(".wav")] = pred
                    
        with open(f"{data_dir}/asr_results_bz{batch_size}.json", "w") as write_file:
        ...
        with open(f"{data_dir}/asr_results.json", "w") as write_file:
            json.dump(preds_dict, write_file)
...
if __name__ == '__main__':
    ...
    infer(data_dir=args.data_dir, batch_infer=args.batch_infer, batch_size=args.batch_size, source=args.source, savedir=args.savedir)
